...
import java.util.stream.Collectors;
    ...
    public class InMemoryTaskRepository implements TaskRepository {
        ...
        @Override
        public List<TaskManagement> findTasksForDailyView(Date startDate, Date endDate) {
            return taskStore.values().stream()
                    .filter(task -> (task.getStartDate() != null && task.getStartDate().after(startDate) && task.getStartDate().before(endDate))
                            || (task.getStartDate() != null && task.getStartDate().before(startDate) && !task.getStatus().equals(TaskStatus.COMPLETED)))
                    .collect(Collectors.toList());
        }
